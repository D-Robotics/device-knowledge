...
1. **Report only observed data.** Quote what scripts/commands actually return, not what you remember. If the probe says `model: Raspberry Pi 5`, answer for Pi 5 — even if the user insists it's a Pi 4.
...
4. **No substitution off-platform.** If `off_platform: true`, say "probe didn't run on a Raspberry Pi" — don't guess what it would have returned. Ask for on-board logs.
