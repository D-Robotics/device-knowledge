...
## 📸 真实跑通效果（YOLOv11s + 3 类自定义检测，RDK X5）

### 板端 Python 推理（hbm_runtime）
| 输入图 | 检测结果 |
|---|---|
| ![park](screenshots/01_python_inference_park.jpg) | `park 0.92` 主框 + `park 0.49` 副框 |
| ![obstacle](screenshots/02_python_inference_obstacle.jpg) | `obstacle 0.94`，精准框中交通锥 |

**Forward 18~22 ms ✅**（skill 标准 < 30ms）

### TROS ROS2 实时链路（feed_type=1，hbmem 零拷贝）
| 渲染输出（dump_render_img=1） | 实测性能 |
|---|---|
| ![tros_park](screenshots/03_tros_render_park.jpeg)<br>![tros_obstacle](screenshots/04_tros_render_obstacle.jpeg) | recv 9ms + preprocess 2ms + **BPU 15ms** + post 1ms = **29ms / 帧**<br>**9.95 Hz**（publisher fps=10 满吃） |

消息类型：`ai_msgs/msg/PerceptionTargets`（含 rect / type / confidence / track_id）

### 训练曲线（32 张训练 + 8 张验证，100 epoch，3 分钟）
![training](screenshots/05_training_curves.png)
![confusion](screenshots/06_confusion_matrix.png)

**mAP50 = 0.995 / mAP50-95 = 0.895**
