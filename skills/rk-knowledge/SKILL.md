...
1. **Report only observed data.** Quote what scripts/commands actually return, not what you remember. If the probe says `soc: rk3588`, answer for RK3588 — even if the user insists it's an RK3576.
...
4. **No substitution off-platform.** If `off_platform: true`, say "probe didn't run on a Rockchip board" — don't guess what it would have returned. Ask for on-board logs.
