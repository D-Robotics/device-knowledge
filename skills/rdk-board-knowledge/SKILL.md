...
description: Identify the RDK board, confirm its runtime baseline (SoC/OS/TROS), diagnose common on-board errors (camera, model/BPU, ROS2, APT, GPIO/I2C/serial, power, network), correct frequent misconceptions, and flash S-series boards with xburn DFU/Fastboot. Use whenever a user pastes an error/log, asks which board this is, reports something broken, asks an official-FAQ question, or needs to flash an S-series image. 触发词:报错、排查、诊断、卡死、连不上、识别板型、系统版本、摄像头没画面、模型跑不了、NO_PUBKEY、GPIO 权限、供电不足、under-voltage、xburn、烧录、DFU、Fastboot、变砖、S100/S600 烧录、官方 FAQ、默认账户密码。Routing — pin/spec facts → rdk-hardware; model deployment → rdk-device; peripheral cookbooks → rdk-peripheral-cookbook; product comparison → rdk-ecosystem; latest doc URL → rdk-doc-finder.
