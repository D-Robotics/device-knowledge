...
1. **Report only observed data.** Quote what scripts/commands actually return, not what you remember. If the probe says `model: Orin Nano`, answer for Orin Nano — even if the user insists it's an AGX Orin.
...
4. **No substitution off-platform.** If `off_platform: true`, say "probe didn't run on a Jetson" — don't guess what it would have returned. Ask for on-board logs.
